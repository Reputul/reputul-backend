...
public interface ChannelCredentialRepository extends JpaRepository<ChannelCredential, Long> {
     ...
     */
    @Query("SELECT c FROM ChannelCredential c WHERE c.metadataJson LIKE %:searchTerm%")
    Optional<ChannelCredential> findByMetadataContaining(@Param("searchTerm") String searchTerm);
